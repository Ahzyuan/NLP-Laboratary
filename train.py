...
save_dir=r'E:\AIL\project\TJ_Palm\Results\saved_model'
log_dir=r'E:\AIL\project\TJ_Palm\Results\train_log'
tb_log_dir=r'E:\AIL\project\TJ_Palm\Results\tb_log'
for _ in [save_dir,log_dir,tb_log_dir]:
    if not os.path.exists(_):
        os.makedirs(_)
struct_time=localtime()
weight_save_path=os.path.join(save_dir, 'best({}\\{}\\{}_{}_{}).pth'.format(*struct_time[:5]))
task_train_log_path=os.path.join(log_dir,'train_log({}\\{}\\{}_{}_{}).txt'.format(*struct_time[:5]))
...
model = nn.parallel.DistributedDataParallel(TJ_model(config),device_ids=[local_rank],output_device=local_rank)
criterion = nn.CrossEntropyLoss()
optimizer  = optim.RMSprop(model.module.parameters(), lr=lr, weight_decay=weight_decay, momentum=momentum)
...
for i in range(epoch):
    ...
    for batch,(img,labels) in enumerate(dataloader.train_loader):
        img,labels = img.to(local_rank), labels.to(local_rank)
        optimizer.zero_grad()
        logit = model(img)
